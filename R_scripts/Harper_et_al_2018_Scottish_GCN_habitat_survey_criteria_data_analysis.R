...
setwd("/Users/Lynsey/Documents/GCN_habitat_survey_critera")
...
## PLOT 2B: Moon phase and GCN counts
...
p2b <- p2b + geom_jitter(aes(x=fPhase, y=GCN, colour=fPhase, shape=Moon_visible), cex=3)
p2b <- p2b + geom_boxplot(aes(x=fPhase, y=fit, colour=fPhase), outlier.size=3, outlier.colour="black", alpha=0.7)
p2b <- p2b + labs(y="", x ="Moon phase") 
p2b <- p2b + coord_cartesian(ylim=c(0,100))
p2b <- p2b + scale_shape_manual(name="Moon visible",
                                values=c(16,17),
                                labels=c("No","Yes"))
p2b <- p2b + scale_colour_manual(guide=FALSE,
                                 values=c("black",
                                          "goldenrod4",
                                          "goldenrod3",
                                          "goldenrod1",
...
## PLOT 2C: Air temperature and GCN counts
...
## Conductivity and pH are set as the mean values in this study
...
## values from model
p2c <- ggplot(GCN.dat) + ggtitle('(c)')
p2c <- p2c + geom_point(aes(x=Air_Temp, y=GCN, colour=Air_Temp, shape=Moon_visible), cex=3)
p2c <- p2c + geom_line(aes(x=dat.AT$Air_Temp, y=dat.AT$fit, colour = dat.AT$Air_Temp), size = 1)
p2c <- p2c + geom_ribbon(aes(x=dat.AT$Air_Temp, ymin = dat.AT$cil, ymax = dat.AT$ciu), alpha = 0.25)
p2c <- p2c + scale_y_continuous(limits=c(0,100))
p2c <- p2c + scale_x_continuous(limits=c(0,12), breaks = c(0,2,4,6,8,10,12))
p2c <- p2c + scale_colour_gradient(limits=c(0, 12), low="slategray4", high="slategray2")
p2c <- p2c + labs(y = "Number of adult great crested newt", x = "Air Temperature ("*degree~"C)")
...
## Conductivity and Air_Temp are set as the mean values in this study
## All factors are set to one level
...
## values from model
p2d <- ggplot(GCN.dat) + ggtitle('(d)')
p2d <- p2d + geom_point(aes(x=pH, y=GCN, colour=pH), cex=3)
p2d <- p2d + geom_ribbon(aes(x=dat.pH$pH, ymin = dat.pH$cil, ymax = dat.pH$ciu), alpha = 0.25)
p2d <- p2d + geom_line(aes(x=dat.pH$pH, y=dat.pH$fit, colour = dat.pH$pH), size = 1)
p2d <- p2d + scale_colour_gradient(limits=c(6,9), low="Green", high="#339999")
p2d <- p2d + scale_y_continuous(limits=c(0,100))
p2d <- p2d + scale_x_continuous(limits=c(6.5,9.0))
p2d <- p2d + labs(y = "", x = "pH")
...
mylegend <- g_legend(p2b)

## Show all plots with common legend
p2a_d <- grid.arrange(arrangeGrob(p2a, p2b + theme(legend.position="none"),
                                  p2c, p2d, nrow=2, ncol=2),
